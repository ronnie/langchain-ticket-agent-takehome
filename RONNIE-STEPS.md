...
- [x] Initialized a local git repo and committed scaffolding (`.gitignore`, `.env.example`, `requirements.txt`, this file). 
  ```bash
  git init
  git add .gitignore .env.example requirements.txt ronnie-steps.txt
  git commit -m "Initial project scaffolding"
  ```
- [ ] Copied the template and filled in real keys (never committed to git):
  ```bash
...
- [ ] Ran smoke test - **failed**: keys are valid/loaded correctly, but the Anthropic API call returned `400 invalid_request_error: Your credit balance is too low to access the Anthropic API`.
  ...
  ```
  Note for walkthrough: a Claude Pro subscription (claude.ai) is separate from Anthropic API billing (console.anthropic.com) - API calls need credits purchased there even with an active Pro plan.
...
- [x] Verified traces landed in the LangSmith UI.
  ...
  Saw both smoke-test runs listed: the failed attempt (`BadRequestError`, $0.00, before credits were added) and the successful one (`ai: Hello`, 18 tokens, $0.000034, 0.94s latency). Confirms tracing is wired correctly.
  Screenshot saved to repo: `evidence/langsmith-smoke-test-traces.png`.
...
- [x] Reviewed results in the LangSmith UI.
...
- [x] Added an evaluator via **+ Evaluator → Correctness** (prebuilt LLM-as-judge, "whether an answer semantically matches reference").
  - Had to reconfigure its judge model too: it defaulted to a different provider (OpenAI) than the main prompt, which would have needed a separate `OPENAI_API_KEY`. Switched it to Anthropic (`claude-sonnet-5`) instead - see Friction Log entry #3, this requires entering the Anthropic key into LangSmith's own **Secrets & API keys** store (separate from local `.env`). Entered the key directly into that LangSmith UI field to get the evaluator working.
...
- [x] Created a private GitHub repo and pushed everything.
  ...
  ```
  Repo: https://github.com/ronnie/langchain-ticket-agent-takehome 
